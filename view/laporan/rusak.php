...
if ($tgl1 == $cektgl1 && $tgl2 == $cektgl2) {
    $sql = $con->query("SELECT * FROM rusak r LEFT JOIN inventaris a ON r.id_inventaris = a.id_inventaris LEFT JOIN ruangan b ON a.id_ruangan = b.id_ruangan LEFT JOIN kategori c ON a.id_kategori = c.id_kategori WHERE r.tgl_rusak BETWEEN '$tgl1' AND '$tgl2' ORDER BY r.tgl_rusak ASC");

    $label = 'LAPORAN KERUSAKAN INVENTARIS <br> Tanggal Kerusakan : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2);
} else {
    $sql = $con->query("SELECT * FROM rusak r LEFT JOIN inventaris a ON r.id_inventaris = a.id_inventaris LEFT JOIN ruangan b ON a.id_ruangan = b.id_ruangan LEFT JOIN kategori c ON a.id_kategori = c.id_kategori ORDER BY id_rusak DESC");
    $label = 'LAPORAN KERUSAKAN INVENTARIS';
}

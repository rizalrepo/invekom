...
if ($tgl1 == $cektgl1 && $tgl2 == $cektgl2) {
    $sql = $con->query("SELECT * FROM mutasi r LEFT JOIN inventaris a ON r.id_inventaris = a.id_inventaris LEFT JOIN ruangan b ON a.id_ruangan = b.id_ruangan LEFT JOIN kategori c ON a.id_kategori = c.id_kategori WHERE r.tgl_mutasi BETWEEN '$tgl1' AND '$tgl2' ORDER BY r.tgl_mutasi ASC");

    $label = 'LAPORAN MUTASI INVENTARIS <br> Tanggal Mutasi : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2);
} else {
    $sql = $con->query("SELECT * FROM mutasi r LEFT JOIN inventaris a ON r.id_inventaris = a.id_inventaris LEFT JOIN ruangan b ON a.id_ruangan = b.id_ruangan LEFT JOIN kategori c ON a.id_kategori = c.id_kategori ORDER BY id_mutasi DESC");
                    ...
                    <tbody>
                        <?php while ($data = mysqli_fetch_array($sql)) { ?>
                            <tr>
                                ...
                                <td align="center">
                                    <?php
                                    $dt = $con->query("SELECT * FROM ruangan WHERE id_ruangan = '$data[id_ruangan_lama]' ")->fetch_array();
                                    echo '<b>' . $dt['nm_ruangan'] . '</b>';
                                    ?>
                                    &nbsp;mutasi ke&nbsp;
                                    <?= '<b>' . $data['nm_ruangan'] . '</b>' ?>
                                </td>
